...
# Adapted from vllm-project/vllm/vllm/worker/enc_dec_model_runner.py
...
from vllm_ascend.worker.model_runner import (NPUModelRunnerBase,
...
class NPUEncoderDecoderModelInput(ModelInputForNPUWithSamplingMetadata):
    """
    ...
    ) -> "NPUEncoderDecoderModelInput":
        return cast(
            NPUEncoderDecoderModelInput,
            super().from_broadcasted_tensor_dict(tensor_dict, attn_backend))


class NPUEncoderDecoderModelRunner(NPUModelRunnerBase[NPUEncoderDecoderModelInput]):
    _model_input_cls: Type[NPUEncoderDecoderModelInput] = (
        NPUEncoderDecoderModelInput)
    ...
    def execute_model(
        self,
        model_input: NPUEncoderDecoderModelInput,
    ...
    def make_model_input_from_broadcasted_tensor_dict(
            self, tensor_dict: Dict[str, Any]) -> NPUEncoderDecoderModelInput:
        return NPUEncoderDecoderModelInput.from_broadcasted_tensor_dict(
            tensor_dict,
    ...
    def _prepare_encoder_model_input_tensors(
        ...
        are used to augment an already-computed `NPUEncoderDecoderModelInput`
